...
Transaction.afterCreate(async (transaction, options) => {
    ...
    if (account) {
        account.balance = transaction.type === 'income' ? Number(account.balance) + Number(transaction.value) : Number(account.balance) - Number(transaction.value);
        await account.save();
    }
});
...
Transaction.beforeUpdate((transaction, options) => {
    // Save the data fields before update
    transaction._previousDataValues = { ...transaction._previousDataValues };
});

// Hook afterUpdate to revert transaction and apply the update one
